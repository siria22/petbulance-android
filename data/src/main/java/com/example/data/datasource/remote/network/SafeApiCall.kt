...
suspend inline fun <reified T, reified R> safeApiCall(
    ...
    try {
        ...
        Log.d("$logger (request)", "path: $path\nrequestDto: ${requestDto?.toString()}")
        ...
        when (statusCode) {
            ...
            in 400..599 -> {
                val errorBody = json.decodeFromString<ErrorResponse<T>>(responseString)
                return Result.failure(
                    mapToDomainException(errorBody.status, path, errorBody.message, requestDto)
                )
            }
        ...
        val errorMessage = "Exception in SafeApiCall for path: $path, DTO: ${requestDto?.toString()}"
        Log.e("$logger (Error)", errorMessage, e)
        return Result.failure(Exception(errorMessage, e))
    }
}

fun mapToDomainException(code: Int, requestPath: String, message: String, requestDto: Any?): Exception {
    val errorMessage = "Request failed for path: $requestPath. DTO: ${requestDto?.toString()}. Reason: $message"
    return when (code) {
